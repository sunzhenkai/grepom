...
`grepom watch` 和 `grepom pipeline watch` SHALL 共享同一 watch 轮询循环实现，不重复实现轮询、状态渲染、Ctrl+C 处理等逻辑。

#### Scenario: watch 循环行为一致
- **WHEN** 用户通过 `grepom watch web-app` 或 `grepom pipeline watch web-app` 监控同一个 pipeline
- **THEN** 两者的轮询间隔、状态行格式、终态退出行为 SHALL 完全一致

#### Scenario: Ctrl+C 行为一致
- **WHEN** 用户在 `grepom watch` 运行过程中按 Ctrl+C
