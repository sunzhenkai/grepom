...
main() {
  ...
  arch="$(detect_arch)"
  tag="$(resolve_tag)"
  asset="grepom_${tag}_${os}_${arch}.tar.gz"
  ...
  if ! install -m 755 "${tmpdir}/grepom" "${INSTALL_DIR}/grepom" 2>/dev/null; then
    err "failed to install to ${INSTALL_DIR}; try: sudo INSTALL_DIR=${INSTALL_DIR} bash"
  fi

  info "Installed grepom ${tag} to ${INSTALL_DIR}/grepom"
