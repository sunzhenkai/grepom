...
系统 SHALL 提供 `grepom scan` 命令，对配置中已克隆的仓库执行敏感信息扫描。命令 SHALL 支持 `--group` 和 `--resource` 标志过滤扫描范围，支持通过位置参数指定单个 repo 名称。未克隆的仓库 SHALL 被跳过并提示。
...
系统 SHALL 以表格形式展示每个发现项，包含以下列：REPO、FILE、LINE、RULE、SEVERITY。表格后 SHALL 输出汇总统计（按严重程度计数）。

#### Scenario: 发现敏感信息时的输出
- **WHEN** 扫描发现敏感信息
- **THEN** 系统输出表格，每行一个发现项，包含仓库名、文件路径、行号、规则 ID 和严重程度标签
...
#### Scenario: 汇总统计
- **WHEN** 扫描完成后存在发现项
- **THEN** 系统在表格后输出总计行，格式为 "Found N findings in M repos." 并附按严重程度的统计
