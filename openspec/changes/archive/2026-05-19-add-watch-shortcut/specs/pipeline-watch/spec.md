...
系统 SHALL 提供 `grepom pipeline watch <repo-name>` 命令，实时监控指定仓库最新一条 pipeline 的运行状态。watch 循环逻辑 SHALL 通过 `WatchTarget` 结构体和 `runWatchLoop()` 函数实现，供 `pipeline watch` 和顶级 `watch` 命令复用。

#### Scenario: watch 最新 pipeline
- **WHEN** 用户运行 `grepom pipeline watch web-app`
- **THEN** 系统 SHALL 通过 `resolvePipelineInput` 获取 provider 信息，构造 `WatchTarget`，调用 `runWatchLoop(target, pipelineID)` 监控最新 pipeline
...
- **THEN** 系统 SHALL 输出最终状态 "Pipeline finished: ✅ success (2m 34s)"，打印 pipeline URL，然后退出

#### Scenario: pipeline 失败
- **WHEN** watch 监控的 pipeline 状态变为 `failed`
- **THEN** 系统 SHALL 输出最终状态 "Pipeline finished: ❌ failed (5m 12s)"，打印 pipeline URL，然后退出

#### Scenario: pipeline 被取消
- **WHEN** watch 监控的 pipeline 状态变为 `canceled`
- **THEN** 系统 SHALL 输出最终状态 "Pipeline finished: 🚫 canceled"，打印 pipeline URL，然后退出
...
- **THEN** 系统 SHALL 停止轮询，输出当前已知的 pipeline 状态，打印 pipeline URL，然后退出（退出码 0）

### Requirement: watch 输出格式

系统 SHALL 在 watch 过程中实时刷新状态行，并在开始和结束时打印 pipeline URL。
...
#### Scenario: watch 启动信息
- **WHEN** watch 开始
- **THEN** 系统 SHALL 先输出启动信息和 pipeline URL：
  ```
  Watching pipeline #1234 for web-app... (Ctrl+C to stop)
    👉 https://gitlab.mycompany.com/.../pipelines/1234
  ```
