...
const DefaultRepo = "sunzhenkai/grepom"
...
func extractBinary(archivePath, dest string) error {
	...
	for {
		...
		if hdr.Typeflag != tar.TypeReg || hdr.Name != "grepom" {
			continue
		}
	...
	return fmt.Errorf("grepom binary not found in archive")
}
...
func Update(opts Options) (*Result, error) {
	...
	if !opts.Force && isUpToDate(opts.Current, targetVersion) {
		...
		logf(opts, "grepom %s is already up to date\n", opts.Current)
		return result, nil
	}
	...
	binaryPath := filepath.Join(tmpDir, "grepom")
	...
	logf(opts, "Installed grepom %s to %s\n", targetVersion, installedTo)
	return result, nil
}
