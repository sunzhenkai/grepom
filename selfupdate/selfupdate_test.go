...
func TestFetchLatestRelease(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/repos/sunzhenkai/grepom/releases/latest" {
			http.NotFound(w, r)
			return
		}
		_ = json.NewEncoder(w).Encode(releaseInfo{TagName: "v0.1.7"})
	}))
	t.Cleanup(srv.Close)
	...
	tag, err := FetchLatestRelease("sunzhenkai/grepom", client)
	if err != nil {
		t.Fatal(err)
	}
	if tag != "v0.1.7" {
		t.Fatalf("got %q, want v0.1.7", tag)
	}
}
...
func TestUpdateInstallsRelease(t *testing.T) {
	...
	asset := "grepom_v0.1.8_" + osName + "_" + arch + ".tar.gz"
	archiveBytes, checksum := buildTestArchive(t, binaryContent)

	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.URL.Path == "/repos/sunzhenkai/grepom/releases/latest":
			_ = json.NewEncoder(w).Encode(releaseInfo{TagName: "v0.1.8"})
	...
	if result.TargetVersion != "v0.1.8" {
		t.Fatalf("target version = %q", result.TargetVersion)
	}

	installed := filepath.Join(installDir, "grepom")
...
func TestVerifyChecksumMismatch(t *testing.T) {
	dir := t.TempDir()
	archive := filepath.Join(dir, "grepom_v0.1.8_linux_amd64.tar.gz")
	if err := os.WriteFile(archive, []byte("bad"), 0o644); err != nil {
		t.Fatal(err)
	}
	checksums := filepath.Join(dir, "checksums.txt")
	if err := os.WriteFile(checksums, []byte("deadbeef  grepom_v0.1.8_linux_amd64.tar.gz\n"), 0o644); err != nil {
		t.Fatal(err)
	}
