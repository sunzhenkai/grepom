...
并行克隆 SHALL 使用 worker pool 模式：主 goroutine 分发任务，N 个 worker goroutine 消费任务并返回结果。

#### Scenario: 仓库数量小于并行度
- **WHEN** 有 2 个仓库需要克隆，`--concurrency` 为 4
- **THEN** 系统仅创建 2 个任务，2 个 worker 各处理一个任务

#### Scenario: 仓库数量大于并行度
- **WHEN** 有 20 个仓库需要克隆，`--concurrency` 为 4
- **THEN** 系统创建 4 个 worker，分批处理 20 个任务

#### Scenario: 单个仓库克隆失败不影响其他仓库
- **WHEN** 并行克隆中某个仓库的所有认证方式均失败
- **THEN** 系统记录该仓库的失败结果，其他 worker 继续处理剩余仓库
