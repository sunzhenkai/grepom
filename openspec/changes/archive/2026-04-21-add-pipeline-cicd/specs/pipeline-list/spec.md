...
系统 SHALL 复用现有 token 优先级链路获取 API 访问令牌。

#### Scenario: Token 解析
- **WHEN** 执行 pipeline list
- **THEN** 系统 SHALL 通过 `repo.Resolver.ResolveAndFilter` 获取已解析的 token（遵循 repo > group > resource 优先级），通过 `cfg.Resources[repo.Resource]` 获取 ServerURL 和 Provider 类型
